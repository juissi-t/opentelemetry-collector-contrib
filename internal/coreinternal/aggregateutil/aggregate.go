...
func mergeExponentialHistogramDataPoints(dpsMap map[string]pmetric.ExponentialHistogramDataPointSlice,
	...
	for _, dps := range dpsMap {
		...
		for i := 1; i < dps.Len(); i++ {
			...
			// Check if offsets are different (indicating offset-aware merging was needed)
			...
			// Only adjust offsets and remove leading zeros if we had different offsets and the buckets are not empty
...
func removeLeadingZeros(buckets pcommon.UInt64Slice) {
	zeroCount := 0
